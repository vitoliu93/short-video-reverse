...
## §12 X3 结果块 — 真实抖音评测 + 结论（✅ 完成，2026-05-30）

**素材**：用户提供 7 条真实抖音（均 1080×1920 / 30fps）：车评×2(老潘说车177s/极速马力135s)、时尚×2(双沐106s/白昼小熊17s)、剧情(阿硬16s)、儿童(半壁江山10s)、AI二次元(失忆19s)。**全量跑 detect**（VLM-free），**VLM 全链路处理子集**：car(135s)/hair(17s)/drama(16s)；**另调查 2 个 0-窗口片**：ai/kid。

**Detect 规模（7 片共 184 候选窗口）**：双沐 72、极速马力 39、老潘说车 48、白昼小熊 20、阿硬 5、半壁江山 0、失忆 0。

**全链路处理结果（k=1，committed 到 outputs/fx/）**：
| 片 | 转场(present) | type 分布 | 特效 | 备注 |
|-|-|-|-|-|
| drama 16s | 5 | hard-cut×5 | particles, shake | 剧情演绎=纯切，正确 |
| hair 17s | 18 | hard-cut×17, none×1(已剔) | 0 | 剪发变身 montage；1 帧抽取失败(已修) |
| car 135s | 36 | hard-cut×32, glitch×4, push×1, slide×1, none×3(已剔) | shake×2, light-leak | 见下人工核 |

**人工核（contact sheet 逐帧看）**：
- **glitch ×2（29.3s / 117.9s）= 正确**：是该博主反复用的 SMPTE 彩条/电视雪花 故障转场（签名手法），VLM 全中。
- **push/slide @101–102 = 误报**：实为相似连续车尾镜头的硬切；**k=3 复跑 3/3 = hard-cut**，committed 里的 push/slide 是 k=1 随机离群。→ motion 类转场在「相似连续镜头」上会被过度解读。
- **hair 密集硬切 = 正确**：同机位镜面自拍、每切一段头发状态（长→短发），内容大变 → 真硬切。
- **none 剔除有效**：hair 1 个、car 3 个候选被 VLM 判 none 正确剔除（VLM 充当候选的投票器）。

**k-投票实证（直接验证 §11 的建议）**：push?@101.591 → 3/3 hard-cut（纠正离群）；glitch@29.258 → 3/3 glitch（稳定）。**结论：type 的 run-to-run 噪声真实存在，但 k=3 多数投票能恢复真值。** 评测/生产应对 type 取 k=3 多数。

**定位（TransNetV2⊕ffmpeg）发现**：
- **静态内容不误报**：AI 片(失忆)是一段连续 AI 生成动漫场景、无镜头边界 → detect=0，正确（不幻觉转场）。
- **召回盲点 = 低内容变化的同机位微切**：儿童片(半壁江山)同人同背景同机位、只换姿势 → 全局像素差太小，transnet+ffmpeg 都=0，漏掉可能的卡点切。**精修结论：盲点取决于「内容变化幅度」而非「是否同机位」**——hair 同机位但内容大变照样检出。这类漏掉的也只是普通硬切，非花式转场。

**成本/延迟（k=1，串行）**：每窗口 ~5–8s。drama 5窗/77s、hair 20窗/189s、car 39窗/425s。抖音长片(135–178s) 39–48 窗口 → ~7–11min/片；k=3 则 ×3。detect 本身 cheap（CPU，135s 片 22s）。

**X3 修的 bug**：(1) full-range YUV 抽帧失败 → extract_frame 加 `-strict unofficial`；(2) 末帧采样失败 → 窗口 t_end 留 0.05s 余量。

### 净结论（plan 收尾）
**实验目标达成：真实抖音爆款的转场/特效确实可被定位、结构化、用自然语言描述并打标签（含闭集 type + 剪映类目），方向成立。**
- 可靠：hard-cut / glitch / fade / dissolve / 闪 等结构清晰的转场；特效(粒子/抖动/漏光/调色)best-effort 可用。
- 需 k=3 投票去噪：glitch↔wipe↔distortion、push/slide 等模糊/运动类 type。
- 已知盲点：渐变 morph（AI 融合）与低内容变化同机位微切的定位召回；纯静态内容正确不误报。
- 后续（超出本 plan）：把 k-投票收进 fx_extract；收紧运动类转场 prompt（要求可见推/滑动画才判 push/slide，否则 hard-cut）；运动类可加光流提示消歧；用这些结构化输出做 ARC 的 RL 微调语料（用户的最终目标）。
