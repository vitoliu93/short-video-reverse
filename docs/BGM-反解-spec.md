...
> 更新:2026-05-29 · 状态:**M0 已验证通过(见 §7)**,CLAP 路线确认,进入 M1
...
- 索引规模 ~1025 向量,faiss `IndexFlatIP`(内积,配 L2 归一化向量)足够,无需量化。
...
- 依赖(M0 已锁定可用版本):`torch`、**`transformers==4.46.3`**(5.x 把 CLAP `get_audio_features` 重构成返回未投影输出且维度错位,4.46 是文档化稳定 API)、`librosa`、`soundfile`、`numpy`;M1 增 `demucs`、`faiss-cpu`。**不用 `laion-clap` pip 包**(依赖地狱),CLAP 走 `transformers` 的 `laion/larger_clap_music`。
...
| **M1 地基** | `bgm_build_index.py` + A + C | 全库索引建成(去均值+去重);单视频能出 volume_profile/start/end/tempo | 已知能跑 |
